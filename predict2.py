...
def main():
    ...
    # Data loading code
    img_ids = glob(os.path.join('inputs/DRAC2022/2.NA/train', 'images', '*' + config['img_ext']))
    img_ids = [os.path.splitext(os.path.basename(p))[0] for p in img_ids]

    # model.load_state_dict(torch.load('models/%s/model.pth' % config['name']))
    # model.load_state_dict(torch.load('models/DRAC2022/2.NA/train_NestedUNet_woDS/model_epoch_200_loss_0.6184_iou_0.3245.pth'))
    ...
    val_dataset = PreDataset(
        img_ids=img_ids,
        img_dir=os.path.join('inputs/DRAC2022/2.NA/train/images'),
    ...
    with torch.no_grad():
        for input, meta in tqdm(val_loader, total=len(val_loader)):
            ...
            for i in range(len(output)):
                cv2.imwrite(os.path.join('inputs/DRAC2022/2.NA/train/prediction', meta['img_id'][i] + '.png'), (output[i, c]*255).astype('uint8'))
    print('complete!')
